...
use ave_actors_actor::{
    ...
    Response, RetryActor, RetryMessage, ShutdownReason, Sink, Strategy,
    Subscriber, SupervisionStrategy,
};
...
// ============================================================================
// Default on_child_error / on_child_fault (actor.rs 475, 483, 488, 496, 501, 503)
// ============================================================================

#[derive(Debug, Clone)]
struct DefaultParentActor;

impl ave_actors_actor::NotPersistentActor for DefaultParentActor {}

#[derive(Debug, Clone, Serialize, Deserialize)]
enum DefaultParentMsg {
    SpawnErrorChild,
    SpawnFaultChild,
}

impl Message for DefaultParentMsg {}

#[async_trait]
impl Actor for DefaultParentActor {
    type Message = DefaultParentMsg;
    type Response = ();
    type Event = SimpleEvent;
    ...
    fn get_span(
        ...
        info_span!("DefaultParentActor", id = %id)
    }
}

#[async_trait]
impl Handler<DefaultParentActor> for DefaultParentActor {
    async fn handle_message(
        &mut self,
        _sender: ActorPath,
        msg: DefaultParentMsg,
        ctx: &mut ActorContext<DefaultParentActor>,
    ) -> Result<(), Error> {
        match msg {
            DefaultParentMsg::SpawnErrorChild => {
                ctx.create_child("err_child", ErrorChildActor).await?;
            }
            DefaultParentMsg::SpawnFaultChild => {
                ctx.create_child("fault_child", FaultyChildActor).await?;
            }
        }
        Ok(())
    }
...
struct SimpleSubscriber {
    events: Arc<Mutex<Vec<SimpleEvent>>>,
}

#[async_trait]
impl Subscriber<SimpleEvent> for SimpleSubscriber {
    async fn notify(&self, event: SimpleEvent) {
        self.events.lock().await.push(event);
    }
}

#[derive(Debug, Clone)]
struct SinkEmitterActor;

impl ave_actors_actor::NotPersistentActor for SinkEmitterActor {}

#[async_trait]
impl Actor for SinkEmitterActor {
    type Message = SimpleMsg;
    type Response = ();
    type Event = SimpleEvent;
    ...
    fn get_span(
        ...
        info_span!("SinkEmitterActor", id = %id)
    }
}

#[async_trait]
impl Handler<SinkEmitterActor> for SinkEmitterActor {
    async fn handle_message(
        &mut self,
        _sender: ActorPath,
        _msg: SimpleMsg,
        ctx: &mut ActorContext<SinkEmitterActor>,
    ) -> Result<(), Error> {
        ctx.publish_event(SimpleEvent(42)).await?;
        Ok(())
    }
}

#[test(tokio::test)]
async fn test_run_sink_helper() {
    let (system, mut runner) =
        ActorSystem::create(CancellationToken::new(), CancellationToken::new());
    tokio::spawn(async move { runner.run().await });

    let actor_ref = system
        .create_root_actor("sink_emitter", SinkEmitterActor)
        .await
        .unwrap();
    ...
    tokio::time::timeout(Duration::from_secs(2), async {
        loop {
            let evts = events.lock().await;
            if !evts.is_empty() {
                assert_eq!(evts[0].0, 42);
                break;
            }
            drop(evts);
            tokio::time::sleep(Duration::from_millis(10)).await;
    ...
    .await
    .expect("sink should receive event");
}

// ============================================================================
// Sink with a genuinely lagged consumer (sink.rs 49-51)
// ============================================================================
...
#[async_trait]
impl Subscriber<SimpleEvent> for SlowSubscriber {
    async fn notify(&self, _event: SimpleEvent) {
...
#[derive(Debug, Clone)]
struct FloodEmitterActor;

impl ave_actors_actor::NotPersistentActor for FloodEmitterActor {}

#[async_trait]
impl Actor for FloodEmitterActor {
    ...
    fn get_span(
        ...
        info_span!("FloodEmitterActor", id = %id)
    }
}

#[async_trait]
impl Handler<FloodEmitterActor> for FloodEmitterActor {
    async fn handle_message(
        &mut self,
        _sender: ActorPath,
        msg: FloodMsg,
        ctx: &mut ActorContext<FloodEmitterActor>,
    ) -> Result<(), Error> {
        for i in msg.batch_start..msg.batch_start + msg.batch_size {
            let _ = ctx.publish_event(SimpleEvent(i)).await;
        }
        Ok(())
    }
}
...
#[test(tokio::test)]
async fn test_sink_lagged_consumer() {
    let (system, mut runner) =
        ActorSystem::create(CancellationToken::new(), CancellationToken::new());
    tokio::spawn(async move { runner.run().await });

    let actor_ref = system
        .create_root_actor("flood", FloodEmitterActor)
        .await
        .unwrap();

    // 2ms delay: slow enough to lag behind the event flood.
    let subscriber = SlowSubscriber { delay_ms: 2 };
    let sink = Sink::new(actor_ref.subscribe(), subscriber);
    system.run_sink(sink).await;

    // Send many small batches so the actor returns to its select loop
    // between batches, allowing the inner channel to drain.
    ...
    // Wait for the actor to finish emitting.
    tokio::time::sleep(Duration::from_millis(100)).await;

    // Stop the actor so the sink receives Closed and exits.
    actor_ref.ask_stop().await.unwrap();

    // Give the sink time to observe the closed channel.
